...
import com.pbu.ui.Helpers.AppConstants;
...
public class LoginServiceImp implements LoginService {
    ...
    public LoginServiceImp(RestTemplate restTemplate) {
        ...
        this.baseUrl = AppConstants.SAM_BASEURL;
    }
    ...
    public LoginModel LoginUser(String username) {
        ...
        SettingsModel timeoutSetting = settings.stream()
                .filter(s -> (AppConstants.TIMEOUT).equals(s.getParamName()))
                .findFirst()
                .orElse(null);
        ...
        if (lastDate != null) {
            SettingsModel daysSetting = settings.stream()
                    .filter(s -> (AppConstants.EXPRIREPWD_DAYS).equals(s.getParamName()))
                    .findFirst()
                    .orElse(null);
